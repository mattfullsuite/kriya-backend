...
const getEmployeeListUsingFilter = (req, res) => {
  ...
  db.query(
    q,
    [payment, compID, from, to, compID, from, to, option, compID, option],
    (err, rows) => {
      if (err) return res.json(err);
      return res.status(200).json(rows);
    }
