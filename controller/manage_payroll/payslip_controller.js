...
const getOffBoardingEmployees = (req, res) => {
  const compID = req.session.user[0].company_id;
  const values = [compID, compID, compID];
  const q = `SELECT e.emp_id, CONCAT( e.f_name, ' ', IF( e.m_name IS NOT NULL AND e.m_name != '', CONCAT(LEFT(e.m_name, 1), '. '), ' ' ), e.s_name ) AS name, e.f_name, e.m_name, e.s_name, e.emp_num, e.work_email, pos.position_name, e.date_hired, e.date_separated, SUM( CAST( JSON_EXTRACT(p.payables, '$.Earnings."Basic Pay"') AS DECIMAL ) ) + SUM( CAST( JSON_EXTRACT(p.payables, '$.Deductions."Absences"') AS DECIMAL ) ) + SUM( CAST( JSON_EXTRACT(p.payables, '$.Deductions."Undertime/Tardiness"') AS DECIMAL ) ) AS total_amount, IFNULL(latest_salary.base_pay, 0) AS base_pay, 0.00 AS 'daily_rate', 0.00 AS 'hourly_rate', rp.recent_payment, rp.recent_date_from, rp.recent_date_to, c.company_name, c.company_loc, cc.monthly_working_days, cc.monthly_payroll_frequency FROM emp e INNER JOIN emp_designation ed ON ed.emp_id = e.emp_id AND e.date_separated IS NOT NULL AND ed.company_id = ? INNER JOIN position pos ON pos.position_id = ed.position_id AND position_name != 'Intern' INNER JOIN company c ON c.company_id = ed.company_id LEFT JOIN ( SELECT company_id, MAX( CASE WHEN configuration_name = 'Monthly Working Days' THEN configuration_value END ) AS 'monthly_working_days', MAX( CASE WHEN configuration_name = 'Monthly Payroll Frequency' THEN configuration_value END ) AS 'monthly_payroll_frequency' FROM primary_app_db.company_configuration WHERE company_id = ? ) AS cc ON cc.company_id = c.company_id LEFT JOIN payslip p ON p.emp_num = e.emp_num AND p.company_id = ? LEFT JOIN ( SELECT emp_id, base_pay FROM emp_salary AS s1 WHERE created_at = ( SELECT MAX(created_at) FROM emp_salary AS s2 WHERE s1.emp_id = s2.emp_id ) ) AS latest_salary ON latest_salary.emp_id = e.emp_id LEFT JOIN ( SELECT emp_num, JSON_UNQUOTE(JSON_EXTRACT(dates, '$.From')) AS recent_date_from, JSON_UNQUOTE(JSON_EXTRACT(dates, '$.To')) AS recent_date_to, JSON_UNQUOTE(JSON_EXTRACT(dates, '$.Payment')) AS recent_payment FROM payslip AS p1 WHERE JSON_EXTRACT(dates, '$.To') = ( SELECT MAX(JSON_EXTRACT(dates, '$.To')) FROM payslip AS p2 WHERE p1.emp_num = p2.emp_num ) ) rp ON e.emp_num = rp.emp_num WHERE p.source != "Last Payrun" GROUP BY e.emp_num ORDER BY e.date_separated DESC`;
