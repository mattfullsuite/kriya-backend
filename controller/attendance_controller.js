...
function GetEmployeesWithStatusForAttendance(req, res){
    ...
    const q = `SELECT DISTINCT e.emp_id, a.employee_id, e.f_name, e.s_name, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Data Incomplete" AND a2.employee_id = a.employee_id) AS data_incomplete, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Late Start" AND a2.employee_id = a.employee_id) AS late_start, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Early Start" AND a2.employee_id = a.employee_id) AS early_start, (SELECT COUNT(*) FROM overtime o INNER JOIN emp eo ON o.requester_id = eo.emp_id WHERE a.employee_id = eo.emp_num) AS overtime, (SELECT COUNT(a2.undertime) FROM attendance a2 WHERE a2.undertime = "Undertime" AND a2.employee_id = a.employee_id) AS undertime, (SELECT COUNT(a3.undertime) FROM attendance a3 WHERE a3.undertime IS NOT NULL AND a3.time_in IS NOT NULL AND a3.time_out IS NOT NULL AND a3.employee_id = a.employee_id) AS completed FROM attendance a INNER JOIN emp e ON a.employee_id = e.emp_num INNER JOIN emp_designation ed ON e.emp_id = ed.emp_id WHERE ed.company_id = ? GROUP BY e.emp_id, a.employee_id, e.f_name, e.s_name`
