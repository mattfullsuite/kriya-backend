      ...
      if (err) {
        ...
        const hw =
          !parseInt(d[4]) || !parseInt(d[5])
            ? null
            : tin < tout
            ? ("0" + parseInt(moment.duration(tout.diff(tin)).asHours())).slice(
                -2
              ) +
              "h " +
              (
                "0" +
                (parseInt(
                  moment.duration(tout.add(1, "days").diff(tin)).asMinutes()
                ) %
                  60)
              ).slice(-2) +
              "m"
            : (
                "0" +
                parseInt(
                  moment.duration(tout.add(1, "days").diff(tin)).asHours()
                )
              ).slice(-2) +
              "h " +
              (
                "0" +
                (parseInt(moment.duration(tout.diff(tin)).asMinutes()) % 60)
              ).slice(-2) +
              "m";

        var st =
          d[4] == null || d[4] == "" || d[5] == null || d[5] == ""
            ? "Data Incomplete"
            : data == null || data == ""
            ? "No Shift Registered"
            : moment(data[0].start, "HH:mm:ss a") > tin
            ? "Early Start"
            : "Late Start";
        ...
        var un =
          d[4] == null || d[4] == "" || d[5] == null || d[5] == ""
            ? null
            : parseInt(moment.duration(tout.diff(tin)).asHours()) >= 8
            ? "Completed"
            : "Undertime";

        var values =
          [d[0], d[3], d[4], convertedOut, hw, st, un];


        const q2 =
          "INSERT INTO attendance (`employee_id`, `date`, `time_in`, `time_out`, `hours_worked`, `status`, `undertime`) VALUES (?)";
...
function GetMyLeaves(req, res) {
  ...
  const q =
    "SELECT leave_id, leave_from, leave_to, use_pto_points FROM leaves WHERE requester_id = ?";
...
function GetEmployeesWithStatusForAttendance(req, res) {
  ...
  const q = `SELECT DISTINCT e.emp_pic, e.emp_id, a.employee_id, e.f_name, e.s_name, es.shift_type, es.start, es.end, p.position_name, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Data Incomplete" AND a2.employee_id = a.employee_id) AS data_incomplete, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Late Start" AND a2.employee_id = a.employee_id) AS late_start, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Early Start" AND a2.employee_id = a.employee_id) AS early_start, (SELECT COUNT(*) FROM overtime o INNER JOIN emp eo ON o.requester_id = eo.emp_id WHERE a.employee_id = eo.emp_num) AS overtime, (SELECT COUNT(a2.undertime) FROM attendance a2 WHERE a2.undertime = "Undertime" AND a2.employee_id = a.employee_id) AS undertime, (SELECT COUNT(a3.undertime) FROM attendance a3 WHERE a3.undertime IS NOT NULL AND a3.time_in IS NOT NULL AND a3.time_out IS NOT NULL AND a3.employee_id = a.employee_id) AS completed FROM attendance a INNER JOIN emp e ON a.employee_id = e.emp_num INNER JOIN emp_designation ed ON e.emp_id = ed.emp_id LEFT JOIN emp_shift es ON es.emp_num = a.employee_id INNER JOIN position p ON p.position_id = ed.position_id WHERE ed.company_id = ? GROUP BY e.emp_id, a.employee_id, e.f_name, e.s_name, es.start, es.end, p.position_name, es.shift_type`;
...
function SearchAttendanceStatus(req, res) {
  ...
  const q = `SELECT DISTINCT e.emp_id,e.emp_pic,a.employee_id, e.f_name, e.s_name, es.shift_type, es.start, es.end, p.position_name, COUNT(case when a.status = 'Data Incomplete' then 1 else null end) AS data_incomplete, COUNT(case when a.status = 'Late Start' then 1 else null end) AS late_start, COUNT(case when a.status = 'Early Start' then 1 else null end) AS early_start, (SELECT COUNT(*) FROM overtime o INNER JOIN emp eo ON o.requester_id = eo.emp_id WHERE a.employee_id = eo.emp_num) AS overtime, COUNT(case when a.undertime = 'Undertime' then 1 else null end) AS undertime, COUNT(case when a.undertime = 'Completed' then 1 else null end) AS completed FROM attendance a INNER JOIN emp e ON a.employee_id = e.emp_num INNER JOIN emp_designation ed ON e.emp_id = ed.emp_id LEFT JOIN emp_shift es ON es.emp_num = a.employee_id INNER JOIN position p ON p.position_id = ed.position_id WHERE CONCAT(e.emp_id, a.employee_id, e.f_name, e.s_name) LIKE ? AND ed.company_id = ? GROUP BY e.emp_id, a.employee_id, e.f_name, e.s_name, es.start, es.end, p.position_name, es.shift_type`;
...
function GetOneStatusForAttendance(req, res) {
  ...
  const q = `SELECT DISTINCT e.emp_id, e.emp_pic, a.employee_id, e.f_name, e.s_name, es.shift_type, es.start, es.end, p.position_name, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Data Incomplete" AND a2.employee_id = a.employee_id) AS data_incomplete, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Late Start" AND a2.employee_id = a.employee_id) AS late_start, (SELECT COUNT(a2.status) FROM attendance a2 WHERE a2.status = "Early Start" AND a2.employee_id = a.employee_id) AS early_start, (SELECT COUNT(*) FROM overtime o INNER JOIN emp eo ON o.requester_id = eo.emp_id WHERE a.employee_id = eo.emp_num) AS overtime, (SELECT COUNT(a2.undertime) FROM attendance a2 WHERE a2.undertime = "Undertime" AND a2.employee_id = a.employee_id) AS undertime, (SELECT COUNT(a3.undertime) FROM attendance a3 WHERE a3.undertime IS NOT NULL AND a3.time_in IS NOT NULL AND a3.time_out IS NOT NULL AND a3.employee_id = a.employee_id) AS completed FROM attendance a INNER JOIN emp e ON a.employee_id = e.emp_num INNER JOIN emp_designation ed ON e.emp_id = ed.emp_id 
...
function GetPaginatedAttendanceOfOne(req, res) {
  ...
  db.query(q1, [employeeNumber], (err, data1) => {
    if (err) {
      ...
      db.query(
        q2,
        [parsedEmployeeNumber, parsedLimit, offset],
        (err, data2) => {
          if (err) {
            return res.json(err);
          } else {
...
function DisableDatesOfOne(req, res) {
  ...
  //const q = "SELECT * FROM attendance a INNER JOIN emp e ON a.employee_id = e.emp_num WHERE a.employee_id = ? ORDER BY date DESC"
  const q =
    "SELECT date FROM attendance WHERE employee_id = ? ORDER BY date DESC";
...
function ChangeAttendanceOfOne(req, res) {
  ...
  db.query(q1, [req.body.emp_num], (err, data) => {
    ...
    if (err) {
      ...
      var hw =
        !moment(tin, "HH:mm:ss a", true).isValid() ||
        !moment(tout, "HH:mm:ss a", true).isValid()
          ? null
          : tin < tout
          ? ("0" + parseInt(moment.duration(tout.diff(tin)).asHours())).slice(
              -2
          ...
          : (
              "0" +
              parseInt(moment.duration(tout.add(1, "days").diff(tin)).asHours())
            ).slice(-2) +
            "h " +
            (
              "0" +
...
function GetPaginatedAttendanceOfMine(req, res) {
  ...
  const q1 =
    "SELECT COUNT(*) AS count FROM attendance WHERE employee_id = ? ORDER BY date DESC";
...
function GetLeavesOfOne(req, res) {
  ...
  //const q = `SELECT * FROM cheer_likes cl INNER JOIN emp e ON e.emp_id = cl.liker_id INNER JOIN emp_designation em ON em.emp_id = e.emp_id INNER JOIN position p ON em.position_id = p.position_id WHERE cheer_post_id = ?`
  const q =
    "SELECT leave_from, leave_type, emp_num FROM leaves l INNER JOIN emp e ON e.emp_id = l.requester_id WHERE e.emp_num = ?";
