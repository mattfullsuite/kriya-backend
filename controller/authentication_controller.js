...
function processLogin(req, res) {
  ...
  db.query(
    ...
    "SELECT e.*, em.*, p.*, d.*, c.*, e2.emp_id AS hasDownline, e2.f_name AS superior_f_name, e2.s_name AS superior_s_name, e2.emp_num AS superior_emp_num FROM emp AS e INNER JOIN emp_designation AS em ON e.emp_id = em.emp_id INNER JOIN position AS p ON em.position_id = p.position_id INNER JOIN dept AS d ON d.dept_id = p.dept_id INNER JOIN company AS c ON c.company_id = em.company_id LEFT JOIN emp e2 ON e.emp_id = e2.superior_id WHERE e.work_email = ? AND e.emp_role != 0 AND e.date_separated IS NULL",
    [work_email],
    (err, result) => {
      if (err) {
        res.send(err);
