...
function GetAllAnonActiveSurveys(req, res) {
  var uid = req.session.user[0].emp_id;
  const q =
    "SELECT * FROM anon_pulse_survey WHERE pulse_survey_id not in (SELECT survey_id FROM anon_pulse_survey_answers WHERE respondent_id = ?) AND survey_status = 1";
  //const q = "SELECT * FROM anon_pulse_survey WHERE survey_status = 1";
...
function InsertAnonSurveyAnswer(req, res) {
  var uid = req.session.user[0].emp_id;
  const values = req.body;
  ...
  const q =
    "INSERT INTO anon_pulse_survey_answers (`respondent_id`, `survey_id`, `answer_body`) VALUES ?";
