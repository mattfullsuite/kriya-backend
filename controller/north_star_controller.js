...
function InsertNorthStar(req, res) {
  ...
  const q =
    "INSERT INTO north_star (`upline_id`, `target_goal`, `target_desc`) VALUES (?)";
...
function EditNorthStar(req, res) {
  ...
  const q =
    "UPDATE north_star SET target_goal = ?, target_desc = ? WHERE north_star_id = ?";
...
function GetMyOwnNorthStar(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "SELECT north_star_id, target_goal, target_desc FROM north_star WHERE upline_id = ?";
...
function GetNorthStarOfMySuperior(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "SELECT * FROM north_star ns INNER JOIN emp e ON ns.upline_id = e.superior_id WHERE e.date_separated IS NULL AND e.emp_id = ?";
...
function GetMyDownlines(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "(SELECT * FROM emp WHERE emp_id = ?) UNION (SELECT child.* FROM emp parent INNER JOIN emp child ON child.superior_id = parent.emp_id WHERE child.date_separated IS NULL AND parent.emp_id = ?)";
...
function InsertNorthStarGoal(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "INSERT INTO north_star_goals (`assigner_id`, `assignee_id`, `target_task`,`target_date`, `status`) VALUES (?)";
  ...
  db.query(q, [values], (err, data) => {
    if (err) {
      ...
      const q = `SELECT nsg.*, 
        assignee.f_name AS assignee_fname,
        assignee.s_name AS assignee_sname,
        assigner.f_name AS assigner_fname,
        assigner.s_name AS assigner_sname
        FROM north_star_goals nsg
        INNER JOIN emp assignee
        ON assignee.emp_id = nsg.assignee_id
        INNER JOIN emp assigner
        ON assigner.emp_id = nsg.assigner_id
        WHERE nsg.assignee_id = ? AND nsg.status != 0 ORDER BY north_star_goal_id DESC LIMIT 1;`;
      db.query(q, [uid], (err, data) => {
        if (err) {
          console.log(err);
          res.send("error");
...
function InsertNorthStarGoalTeam(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "INSERT INTO north_star_goals (`assigner_id`, `assignee_id`, `target_task`,`target_date`, `status`) VALUES (?)";
  ...
  db.query(q, [values], (err, data) => {
    if (err) {
      ...
      assignee.s_name AS assignee_sname
          FROM north_star_goals nsg
          INNER JOIN emp assigner ON assigner.emp_id = nsg.assigner_id 
          INNER JOIN emp assignee ON assignee.emp_id = nsg.assignee_id 
          WHERE (assigner_id = ? AND assignee_id != ? AND nsg.status != 9 AND nsg.status != 0) 
          OR (assignee.superior_id = ? AND (nsg.status !=9 AND nsg.status != 0)) ORDER BY north_star_goal_id DESC LIMIT 1`;
...
function GetTaskOfSameLine(req, res) {
  ...
  assignee.s_name AS assignee_sname
      ...
      INNER JOIN emp assignee ON assignee.emp_id = nsg.assignee_id 
      WHERE (assigner_id = ? AND assignee_id != ? AND nsg.status != 9 AND nsg.status != 0) 
      OR (assignee.superior_id = ? AND (nsg.status !=9 AND nsg.status != 0))`;
  db.query(q, [uid, uid, uid], (err, data) => {
    if (err) {
      console.log(err);
      res.send("error");
...
function GetMyTasks(req, res) {
  ...
  INNER JOIN emp assigner
  ON assigner.emp_id = nsg.assigner_id
  WHERE nsg.assignee_id = ? AND nsg.status != 0;`;
  db.query(q, [uid], (err, data) => {
    if (err) {
      console.log(err);
      res.send("error");
...
function GetTasksYouAssigned(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "SELECT * FROM north_star_goals nsg INNER JOIN emp e ON nsg.assignee_id = e.emp_id WHERE nsg.assigner_id = ?";
...
function GetTasksForReview(req, res) {
  ...
  const q =
    "SELECT nsg.*, assigner.f_name AS assigner_fname, assigner.s_name AS assigner_sname, assignee.f_name AS assignee_fname, assignee.s_name AS assignee_sname FROM north_star_goals nsg INNER JOIN emp assigner ON assigner.emp_id = nsg.assigner_id INNER JOIN emp assignee ON assignee.emp_id = nsg.assignee_id WHERE (assigner_id = ? AND assignee_id != ? AND nsg.status = 9) OR (assignee.superior_id = ? AND (nsg.status = 9))";
...
function GetMyTeamTasksYouAssigned(req, res) {
  const uid = req.session.user[0].emp_id;
  const q =
    "SELECT * FROM north_star_goals nsg INNER JOIN emp e ON nsg.assignee_id = e.emp_id WHERE nsg.assigner_id = ? AND nsg.status < 8";
...
function UpdateTaskStatus(req, res) {
  ...
  const normal_q =
    "UPDATE north_star_goals SET status = " +
    req.body.status +
    " WHERE north_star_goal_id = " +
    req.body.north_star_goal_id;

  const finished_q =
    "UPDATE north_star_goals SET status = " +
    req.body.status +
    ", finished_date = CURRENT_TIMESTAMP WHERE north_star_goal_id = " +
    req.body.north_star_goal_id;

  db.query(req.body.status == 0 ? finished_q : normal_q, (err, data) => {
    if (err) {
      res.send("error");
      console.log(err);
    } else {
...
function GetFinishedTaskOfSameLine(req, res) {
  ...
  //const q = "SELECT * FROM north_star_goals nsg INNER JOIN emp e ON nsg.assigner_id = e.emp_id WHERE assignee_id = ?"
  //   const q =
  //     "(SELECT nsg.*, a.f_name AS a_fname, a.s_name AS a_sname, r.f_name AS r_fname, r.s_name AS r_sname FROM north_star_goals nsg INNER JOIN emp a ON nsg.assignee_id = a.emp_id INNER JOIN emp r ON nsg.assigner_id = r.emp_id WHERE assigner_id = ? AND status = 0)";
  ...
  const q =
    "SELECT nsg.*, assignee.f_name AS assignee_fname, assignee.s_name AS assignee_sname, assigner.f_name AS assigner_fname, assigner.s_name AS assigner_sname FROM north_star_goals nsg INNER JOIN emp assignee ON assignee.emp_id = nsg.assignee_id INNER JOIN emp assigner ON assigner.emp_id = nsg.assigner_id WHERE (nsg.assigner_id = ? OR assignee.superior_id = ?) AND nsg.status = 0";
...
function GetMyFinishedTasks(req, res) {
  ...
  //const q = "SELECT * FROM north_star_goals nsg INNER JOIN emp e ON nsg.assigner_id = e.emp_id WHERE assignee_id = ?"
  const q =
    "SELECT nsg.*, assignee.f_name AS assignee_fname, assignee.s_name AS assignee_sname, assigner.f_name AS assigner_fname, assigner.s_name AS assigner_sname FROM north_star_goals nsg INNER JOIN emp assignee ON assignee.emp_id = nsg.assignee_id INNER JOIN emp assigner ON assigner.emp_id = nsg.assigner_id WHERE nsg.assignee_id = ? AND nsg.status = 0;";
...
function GetApprover(req, res) {
  ...
  const q =
    "SELECT * FROM emp AS e INNER JOIN emp AS em ON e.superior_id = em.emp_id WHERE e.emp_id = ?";
...
UNION
  ...
  db.query(q, [uid, uid, uid, uid, uid, uid], (err, data) => {
    if (err) {
      console.log(err);
      res.send("error");
    } else {
