...
app.post("/addHoliday", (req, res) => {
  ...
  const values = [req.body.h_name, req.body.h_date];
...
app.get("/getDirectory", (req, res) => {
  const q =
    "SELECT * FROM emp INNER JOIN emp_designation ON emp.emp_id = emp_designation.emp_id INNER JOIN position ON emp_designation.position_id = position.position_id INNER JOIN dept ON dept.dept_id = position.dept_id INNER JOIN division ON division.div_id=dept.div_id";
